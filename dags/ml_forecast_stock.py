...
def execute_snowflake_with_retry(sql_statements: list[str], conn_id: str = "snowflake_default", max_retries: int = 3) -> bool:
...
def create_training_view(**context):
    ...
    try:
        success = execute_snowflake_with_retry(sql_statements)
        if success:
...
def run_forecast_with_fallback(**context):
    ...
    try:
        success = execute_snowflake_with_retry(primary_sql, max_retries=2)
        if success:
            logger.info("Forecast executed successfully")
            return "success"
        else:
        ...
        execute_snowflake_with_retry(fallback_sql)
...
def build_union_table_robust(**context):
    ...
    try:
        success = execute_snowflake_with_retry(union_sql)
        if success:
...
with DAG(
    dag_id="ml_forecast_stock_robust",
    description="Snowflake ML Forecast pipeline with Variable-based configuration",
    default_args=default_args,
    schedule_interval="@daily",
    start_date=days_ago(1),
    catchup=False,
